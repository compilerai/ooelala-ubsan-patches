...
Address CodeGenFunction::EmitExtVectorElementLValue(LValue LV) {
  ...
  Address VectorBasePtrPlusIx = Builder.CreateConstInBoundsGEP(
      CastToPointerElement, ix, getContext().getTypeSizeInChars(EQT),
      "vector.elt");
...
LValue CodeGenFunction::EmitDeclRefLValue(const DeclRefExpr *E) {
  ...
  assert(
      (ND->isUsed(false) || !isa<VarDecl>(ND) || !E->getLocation().isValid()) &&
...
static Address emitAddrOfFieldStorage(CodeGenFunction &CGF, Address base,
  ...
  // is zero.
  assert((idx != 0 || CGF.getContext().getASTRecordLayout(rec).getFieldOffset(
                          field->getFieldIndex()) == 0) &&
...
LValue CodeGenFunction::EmitConditionalOperatorLValue(
  ...
  if (lhs && rhs) {
    llvm::PHINode *phi =
        Builder.CreatePHI(lhs->getPointer()->getType(), 2, "cond-lvalue");
